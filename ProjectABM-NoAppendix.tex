...
% Given the terminology developed earlier, the natural designation for the stable ratio of aggregate market resources to noncapital income is the `aggregate steady state' ratio, $\StE{M}$.  This is not numerically the same as the individual \href{https://econ-ark.github.io/BufferStockTheory/#pseudo-steady-state}{pseudo-steady-state} ratio $\mStE$ because the nonlinearities involved in simulation and aggregation will have consequences.\footnote{Still, the pseudo-steady-state can be calculated immediately from the policy function without any simulation, and therefore would likely serve as an excellent and low-cost starting point for the numerical simulation process.}

\cite{harmenbergInvariant} shows\footnote{The test of the Harmenberg method currently implemented in the Econ-ARK toolkit shows that, for the same computational resources, Harmenberg's method reduces the standard error of the simulated aggregate consumption series by a factor of more than 8; see the last part of \href{https://github.com/econ-ark/BufferStockTheory/blob/master/reproduce/computed.sh}{\texttt{reproduce/computed.sh}}.} that his reformulation of the problem can reduce costs of calculation enormously.  % The idea also appears to be promisingly generalizable: For any statistic $\mathcal{S}$ that is the key focus of an analysis, it should be possible to improve the efficiency of simulations aimed at calculating that statistic by constructing an $\mathcal{S}$-weighted distribution rather than a neutral distribution.
...
\begin{comment}
\newcommand{\popn}{\Xi}
\newcommand{\Dstn}{\mathcal{F}}
\newcommand{\dstn}{f}
Define $\popn(\mRat)$ as the population density at a given $\mRat$.
...
\begin{align*}
%  \CLevBF_{t+1} & = \Mean_{t+1}[\cRat_{t+1}\pLevBF_{t+1}] = \Mean_{t+1}[\cRat_{t+1}\PGro\pShk_{t+1}\pLevBF_{t}] + \cv_{t+1}(\cRat_{t+1}, \pLevBF_{t}\PGro \pShk_{t+1})
%\\  & = \PGro \Harm{\Mean}_{t+1}[\cRat_{t+1}]\underbrace{\Harm{\Mean}[\pLevBF_{t}]}_{=1} + \cv_{t+1}(\cRat_{t+1}, \pLevBF_{t+1})                      
% \\   \CLevBF_{t+1} & = \Harm{\Mean}_{t+1}[\cRat_{t+1}\pLevBF_{t+1}] = \Harm{\Mean}_{t+1}[\cRat_{t+1}]\Harm{\Mean}_{t+1}[\pLevBF_{t+1}] + \HarmWide{\cv}_{t+1}(\cRat_{t+1}, \pLevBF_{t+1}).
%0 = \CLevBF_{t+1}-\PGro \CLevBF_{t} & =  \Mean[\cRat_{t+1}]\PGro + \PGro \cov_{t+1} - (\Mean[\cRat_{t}]+\cov_{t}) \\ 
%\left(\Mean[\cRat_{t+1}] \PGro+ \cov_{t+1} \right) & = \left(\Mean[\cRat_{t}]+\cov_{t}\right)  \\ 
%\left(\Mean[\cRat_{t+1}]  - \Mean[\cRat_{t}]\right)\PGro & =\cov_{t+1}  - \PGro \cov_{t} \label{eq:MeanaRatVsCovGro}%
%  \\ \left(\Mean[\cRat_{t+2}]\PGro^{2}  - \Mean[\cRat_{t+1}]\PGro^{1}\PGro^{1}\right) & = \cov_{t+2}  - \PGro \cov_{t+1} \label{eq:MeanaRatVsCovGro2}%
%  \\ \left(\Mean[\cRat_{t+2}]\PGro^{2}  - \Mean[\cRat_{t+1}]\PGro^{2}\right) & = \cov_{t+2}  - \PGro \cov_{t+1} \label{eq:MeanaRatVsCovGro2}%
 \left(\bar{\cRat}_{t+2} - \bar{\cRat}_{t+1}\right)\PGro^{2} & = \cov_{t+2}  - \PGro \cov_{t+1}. \label{eq:MeanaRatVsCovGro2}%                                                                                                                                                                       .
%  \left(\tilde{\Mean}[\cRat_{t+1}]/\PGro  - \tilde{\Mean}[\cRat_{t}]\right)\PGro & =\cov_{t+1}  - \PGro \cov_{t} \\
%  \bar{\tilde{\cRat}} \left(1  - \PGro\right) & =\cov_{t+1}  - \PGro \cov_{t}
\end{align*}
...
\begin{comment}
  \begin{align*}
    \overbrace{\Grw_{\cRat}^{n} \bar{\cRat}_{t}}^{\bar{\cRat}_{t+n}}(\Grw_{\cRat} - 1)\PGro^{n} & =(\Grw_{\cv} - \PGro) \overbrace{\cv_{t}\Grw_{\cv}^{n}}^{\cv_{t+n}}
\\                                                                                                 
    (\Grw_{\cRat}\PGro/\Grw_{\cv})^{n} \bar{\cRat}_{t}(\Grw_{\cRat} - 1) & =(\Grw_{\cv} - \PGro) \cv_{t}
  \end{align*}
    and a corresponding equation will hold in $t+1$:
  \begin{align*}
\\  \overbrace{\Grw_{\cRat} \bar{\cRat}_{t}}^{\bar{\cRat}_{t+1}}(\Grw_{\cRat} - 1)\PGro & =(\Grw_{\cv} - \PGro) \overbrace{\cv_{t}\Grw_{\cv}}^{\cv_{t+1}}
  \end{align*}
  \begin{align*}
  (\Grw_{\cRat} - 1)\bar{\cRat}_{t}(\Grw_{\cRat} - 1)\PGro & =(\Grw_{\cv} - \PGro) \cv_{t}(\Grw_{\cv} - 1)
\end{align*}
...
\end{align*}
\begin{align}
%  \Grw_{\cRat} - 1 & = \Grw_{\cv} - \PGro \\
%  \Grw_{\cRat} + \pGro & = \Grw_{\cv}  \\
  \Grw_{\cv} & = \Grw_{\cRat} + (\PGro - 1) \label{eq:aRatGroVscov}
\end{align}
\begin{align*}
  ...
  \CLevBF_{t+1} & = \Harm{\Mean}_{t         +1 }[\cRat]         \PGro  = \bar{\cRat}_{t+1}\PGro  + \cv_{t+1}(\cRat_{t+1}, \pLevBF_{t+1})
\end{align*}

Still, both of these quantitites are in priniciple measurable in micro data.  %Indeed, in principle, the Harmenberg-invariant distribution itself could be measured in such data.
...
%One concern about this point is that the Harmenberg-measure covariance $\HarmWide{\cv}$ is a rather peculiar object, and probably one that would be difficult to measure.  Recall that what we need, for the Harmenberg measure, is to know the total amount of permanent income accruing to persons whose asset-to-permanent-income \emph{ratio} is equal to each particular value.  The concern is alleviated by noting that, for an economy that has converged according to the Szeidl criteria, the same argument can be applied with respect to the standard ways of computing the covariance.
